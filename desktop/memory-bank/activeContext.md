# Buse kişiselleştirmesi — 2026-09-07

Mac güncellemesi tamamlandı: önceden kurulu Türkçe kaynaklar `desktop/` altında korunuyor. Kullanıcı mevcut uygulamayı güncellememizi ve tüm kodu kendi GitHub main dalına göndermemizi açıkça istedi. Kimlik/data path değişmiyor. Yerel ana ekran özel Sites panelini varsayılan tarayıcıda açıyor. Upstream binary updater kapalı. npm run check: 0 hata/uyarı. 29 updater testi geçti. Native macOS paketi başarıyla derlendi, ad hoc imza doğrulandı ve /Applications altındaki 0.3.0 uygulaması yedeklenerek 0.3.1 ile değiştirildi. Açılışta yeni dashboard, 273 ajan ve doğru panel URL’sine tarayıcı geçişi doğrulandı. Sürüm 0.3.1 ad hoc yerel build; AI bağlantısı ayrı ve henüz kullanıcı girişine bağlı.

Aşağıdaki metin upstream tarihçesidir; kişiselleştirilmiş sürümün yayın durumu değildir.
