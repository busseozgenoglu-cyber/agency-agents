# Buse’nin Ajansı — Mac uygulaması

Bu dizin, bilgisayarda kullanılan Türkçe masaüstü uygulamasının kaynak kodudur. Sürüm **0.3.1**; mevcut ajanlar, ekipler, projeler, Türkçe çeviriler ve özel simgeler korunur.

Ana ekrandaki **Ajansımı aç** düğmesi [Buse’nin özel ajans panelini](https://busenin-ajansi.ozgenoglubuse.chatgpt.site/) varsayılan tarayıcıda açar. Personel ekleme, yapay zekâ ekiplerinin proje üzerinde çalışması ve sonuç sunumu web panelindedir. Gerekirse panelin sahibi olan ChatGPT hesabıyla giriş yapılır. Web panelindeki gerçek yapay zekâ çalışmaları ayrıca sunucu tarafında OpenAI bağlantısı gerektirir; uygulamayı güncellemek bu bağlantının kurulduğu anlamına gelmez.

## Mac üzerinde derleme

Node.js 22+, Rust stable ve Xcode Command Line Tools gerekir:
...
Çıktı: `src-tauri/target/release/bundle/macos/Buse'nin Ajansı.app`.

Yerel paket ad hoc imzalanır. Apple notarization veya herkese açık dağıtım imzası içermez. Mevcut `com.zerologic.agency-agents-app` uygulama kimliği korunur; uygulamanın kayıtlı ekipleri, projeleri ve katalog ayarları aynı veri dizinini kullanır. Güncellemeden önce mevcut `.app` paketinin yedeğini alın, uygulamadan çıkın, yeni paketi `/Applications` içine kopyalayın ve yeniden açın. Kullanıcı verilerini silmeyin.

Kişiselleştirmelerin upstream uygulamayla üzerine yazılmaması için upstream uygulama güncelleyicisi bu sürümde kapalıdır. Web paneli kendi adresinde güncellenir; masaüstü güncellemeleri bu depodan derlenir. `scripts/release.sh`, bu kopyanın değil özgün projenin yayın sürecidir; yerel build için yukarıdaki komutu kullanın.

## Kaynak ve lisans

Kaynak, MIT lisanslı [msitarzewski/agency-agents-app](https://github.com/msitarzewski/agency-agents-app) projesinin `04c10beee0f35ed43f88de9f232f1d885a03156c` commit’inden ve önceden bilgisayarda bulunan Türkçe/Buse özelleştirmelerinden alınmıştır. Orijinal lisans, yazar atıfları ve aşağıdaki upstream belgeleri korunur. Derleme çıktıları, kullanıcı verileri ve kimlik bilgileri depoya eklenmez.
