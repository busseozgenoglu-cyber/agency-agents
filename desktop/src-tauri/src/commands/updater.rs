//! Updater implementation retained for a future personalized release channel.
//!
//! This build disables the public check/install entry points, does not register
//! the upstream plugin, and does not start its scheduler. Existing settings or
//! stale frontend calls must never replace the personalized application bundle.
//!
...
fn personalized_build_update_error() -> AppError {
    AppError::InvalidArgument {
        message: "Buse’nin Ajansı kişisel sürümünde uygulama içi güncelleme desteklenmiyor. Kişiselleştirmeleri korumak için bu sürüm özel olarak yeniden derlenip kurulmalıdır.".into(),
    }
}
...
pub async fn update_check_now(
    ...
    let _ = (app, state);
    Err(personalized_build_update_error())
}
